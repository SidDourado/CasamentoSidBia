...
// Seleciona os elementos que serão clicados
const qrCodeImage = document.getElementById('imgPix');
const pixNumberSpan = document.getElementById('numPix');
const tooltip = document.getElementById('tooltip');

// Define a chave PIX a ser copiada (sem formatação)
const chavePix = "18988269060";

// Função para copiar a chave e mostrar o tooltip
function copiarChavePix() {
    // Usa a API do navegador para copiar o texto
    navigator.clipboard.writeText(chavePix).then(() => {
        // Se a cópia for bem-sucedida:
        // 1. Mostra o tooltip adicionando a classe 'show'
        tooltip.classList.add('show');

        // 2. Esconde o tooltip após 3 segundos (3000 milissegundos)
        setTimeout(() => {
            tooltip.classList.remove('show');
        }, 3000);

    }).catch(err => {
        // Caso ocorra um erro ao copiar
        console.error('Falha ao copiar a chave PIX: ', err);
        alert('Não foi possível copiar a chave PIX.');
    });
}

// Adiciona o evento de clique à imagem e ao número
qrCodeImage.addEventListener('click', copiarChavePix);
pixNumberSpan.addEventListener('click', copiarChavePix);